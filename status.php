...
echo "<div class=\"hh text-center\"><h1>Status</h1></div>";
// $hr->toolbar();
echo "<div class=\"btn-toolbar\" role=\"toolbar\" aria-label=\"Liszt Toolbar\"><div class=\"btn-group me-2\" role=\"group\" aria-label=\"Deliveries\"><a href=\"/status/\" class=\"btn btn-outline-liszt btn-lg active\" title=\"Index\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"><i class=\"fa-light fa-square-list\"></i></a><a href=\"/status/new/\" class=\"btn btn-outline-liszt btn-lg\" title=\"New\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"><i class=\"fa-light fa-plus\"></i></a></div></div><br />";
				...
				<br />
					<table class="table table-hover">
						...
						<tr><td>
							...
							<label for="<?php echo $hr->page;?>-status">Status</label>
							<input type="hidden" name="status" value="" />
							<select id="<?php echo $hr->page;?>-status" name="status" class="form-control lz-field">
								<?php
								foreach($hr->statuscodes as $k=>$v)
								{
									echo "<option value=\"" . $k . "\""; if($hr->row['status']==$k){echo " selected";} echo ">" . $v . "</option>";
								}
								?>
								</select>
							</div>
						...
						<?php /*<tr><td>
							<div class="form-group form-group-lg">
							<label for="<?php echo $hr->page;?>-type">Type</label>
							<input type="hidden" name="type" value="" />
							<select id="<?php echo $hr->page;?>-type" name="type" class="form-control lz-field">
								<?php
								foreach($hr->typecodes as $k=>$v)
								{
									echo "<option value=\"" . $k . "\""; if($hr->row['type']==$k){echo " selected";} echo ">" . $v . "</option>";
								}
								?>
								</select>
							</div>
						</td></tr>*/?>
						<tr><td><?php $hr->textarea("data","Data");?></td></tr>
						<script>
							var simplemde = new SimpleMDE({ element: document.getElementById("status-data-field") });
						</script>
						<tr><td><?php $hr->textinput("tags","Tags (separated by comma)");?></td></tr>
